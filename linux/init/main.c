...
/* Experimental code - Not part of original RPi Linux Kernel repo */
static DECLARE_WAIT_QUEUE_HEAD(myevent_waitqueue);
rwlock_t myevent_lock;
extern unsigned int myevent_id; // Holds the identity of the troubled data structure. Populated later on.
...
static int mykthread(void *unused)
{
	...
	for(;;){
		...
		/* Controls gets here when the thread is woken up */
		...
		if(myevent_id){		/* Guard against spurious wakeups */
			...
			/* Invoke the registered user mode helper and pass the identity code in its environment */
