...
Mutex::Mutex ()
{
    ...
    if (res != 0)
    {
        NXPLOG_API_D ("Mutex::Mutex: fail init; error=0x%X", res);
    }
}
...
Mutex::~Mutex ()
{
    ...
    if (res != 0)
    {
        NXPLOG_API_D ("Mutex::~Mutex: fail destroy; error=0x%X", res);
    }
}
...
void Mutex::lock ()
{
    ...
    if (res != 0)
    {
        NXPLOG_API_D ("Mutex::lock: fail lock; error=0x%X", res);
    }
}
...
void Mutex::unlock ()
{
    ...
    if (res != 0)
    {
        NXPLOG_API_D ("Mutex::unlock: fail unlock; error=0x%X", res);
    }
}
...
bool Mutex::tryLock ()
{
    ...
    if ((res != 0) && (res != EBUSY))
    {
        NXPLOG_API_D ("Mutex::tryLock: error=0x%X", res);
    }
    return res == 0;
}
