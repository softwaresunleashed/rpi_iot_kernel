...
CondVar::CondVar ()
{
    ...
    if (res)
    {
        NXPLOG_API_D ("CondVar::CondVar: fail init; error=0x%X", res);
    }
}
...
CondVar::~CondVar ()
{
    ...
    if (res)
    {
        NXPLOG_API_D ("CondVar::~CondVar: fail destroy; error=0x%X", res);
    }
}
...
void CondVar::wait (Mutex& mutex)
{
    ...
    if (res)
    {
        NXPLOG_API_D ("CondVar::wait: fail wait; error=0x%X", res);
    }
}
...
bool CondVar::wait (Mutex& mutex, long millisec)
{
    ...
    {
        NXPLOG_API_D ("CondVar::wait: fail get time; errno=0x%X", errno);
    }
    else
    {
    ...
    {
        NXPLOG_API_D ("CondVar::wait: fail timed wait; error=0x%X", waitResult);
    }
...
void CondVar::notifyOne ()
{
    ...
    if (res)
    {
        NXPLOG_API_D ("CondVar::notifyOne: fail signal; error=0x%X", res);
    }
}
